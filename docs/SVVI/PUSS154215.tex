...
%\item The sensor name and mac-address is shown in the top of the view (Req. 5.3.2, 5.3.3).
	...
	\item Test of on/off switch.
		...
		\begin{instruction}
			...
			\item Wait for ten minutes
			\item Press the "Get All"-button
			\item Check the error message \label{label_instruction_on_off_switch}
			\item Set the on/off-status switch to on
			\item Press the "Get All"-button
			\item Check the text fields
	  	\end{instruction}
	  	\begin{postcondition}
	  		\item The on/off-status switch of the selected sensor is on
	  		\item An error message with the text "Error Occurred" was displayed in instruction \ref{label_instruction_on_off_switch} 
	  		\item Text fields have values
	  	\end{postcondition}
