...
%\newcommand[1]{\precondition}{\textbf{Precondition:}#1}
...
\begin{testlist}
    ...
    %The first screen that is shown on startup is the MyDevicesView (Req. 5.2.1).
   ...
   \item \vspace{5mm} \textbf{Precondition:} The application is running and the MyDevices View is shown, at least two devices are displayed in the list (instruktion?). One device is selected (instruktion?).
                ...
                % Räcker det med att testa bakgrundfärg eller måste man även prova att trycka på "Control device"??
...
\begin{testlist}
 \item \vspace{5mm} \textbf{Precondition:} The MyDevices View is opened and a sensor device populates the list.
    			\begin{enumerate}
    				...
    				%\item Look at the screen and confirm that the Sensor view is opened.
    			\end{enumerate}
